						...
						<el-card class="box-card">
							...
							<ul>
								<li><a href="https://lpc.opengameart.org/" rel="nofollow noopener"
										referrerpolicy="origin">About the
										Liberated Pixel Cup</a></li>
	...
	export default {
		...
		methods: {
			...
			async choseIconHandler(activeCode, type) {
				...
				if (type == "") {
					//清除当前图层
					let layerInfo = this.jsonData.filter(x => x.code == activeCode)[0];
					
					this.clearAniByLayer(layerInfo.layer);
